import converter
import measurer.edit_distance_measurer as measurer
...
def fix(input_word):
    ...
    input_word = converter.cji_converter.convert(input_word)

    ret = set()

    if input_word in cji_dict.keys():
        ret.add(converter.han_converter.convert(input_word))
        print('단어사전에 입력 키워드가 있는 예시', input_word)

    if input_word in delete_dict.keys():
        for keyword in delete_dict[input_word]:
            ret.add(converter.han_converter.convert(keyword))
    ...
    for input_word_del in del_converter.deletes(input_word):
        if input_word_del in cji_dict.keys():
            ret.add(converter.han_converter.convert(input_word_del))
    ...
    for r in print_arr:
        print('단어: ' + r[0] + ')', end = ' ')
        print('물리적거리: %.2f' % measurer.getPhysEditDist(input_word, cji_converter.cheonjiin_convert(r[0])), end = ' ')
        print('빈도수: ', r[1])
    print("------------------")


if __name__ == '__main__':
    dictionaryFileName = "converter/dict/mungchi_dict.txt"
