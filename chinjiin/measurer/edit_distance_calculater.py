...
def calc_edit_dist(str_dict, str_input):
    ...
    for i in range(3, len(str_dict)+2):
        table[i][1] = (table[i-1][1]
                       + get_phys_dist(str_dict[i-2], str_dict[i-3]))
    for i in range(3, len(str_input)+2):
        table[1][i] = (table[1][i-1]
                       + get_phys_dist(str_input[i-2], str_input[i-3]))
    ...
    for i in range(1, len(str_dict)+1):
        ...
        for j in range(1, len(str_input)+1):
            ...
            # Substitution
            val_sub = table[i][j] + cost
            # Addition, distance from previous character
            val_add = (table[i + 1][j]
                       + (table[1][j+1] - table[1][j]))
            # Deletion, doesn't use physical distance
            val_del = (table[i][j+1]
                       + float(1))
            # Transposition
            val_trs = (table[last_match_row][last_match_col]
                       + max((i-last_match_row-1),
                             (j-last_match_col-1))
                       + float(1))

            # determining value of current cell
